(()=>{
  ...
  function boot(){if(booted)return;map=evalPage('map');if(!map||!window.naver?.maps?.Map){setTimeout(boot,200);return}booted=true;stopOriginalMarkers();installCss();shell();readCarrier();restoreView();try{naver.maps.Event.trigger(map,'resize')}catch(e){};setTimeout(()=>{try{naver.maps.Event.trigger(map,'resize')}catch(e){}},300);let stopRuns=0,stopTimer=setInterval(()=>{stopOriginalMarkers();if(++stopRuns>=12)clearInterval(stopTimer)},900);naver.maps.Event.addListener(map,'idle',saveView);naver.maps.Event.addListener(map,'zoom_changed',saveView)}
